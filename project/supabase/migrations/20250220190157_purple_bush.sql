...
CREATE POLICY "contracts_nurse_update"
  ...
  WITH CHECK (
    ...
    (
      (nurse_rating IS NOT NULL AND nurse_review IS NOT NULL) OR
      (nurse_notes IS NOT NULL)
    )
  );
...
CREATE POLICY "contracts_company_update"
  ...
  WITH CHECK (
    company_id = auth.uid() AND
    (
      (company_rating IS NOT NULL AND company_review IS NOT NULL) OR
      (company_notes IS NOT NULL)
    )
  );
